...
class FuncTestGen {
    ...
    setMockData(node, mockData, returnValue) {
        ...
        if ([
            ...
            node.declarations.forEach(decl => {
                ...
                const declReturn = util_1.Util.getObjFromVarPattern(decl.id);
            ...
            const funcReturn = util_1.Util.getFuncReturn(kode);
    ...
    setMockDataMap(node, mockData) {
        ...
        nodeLeft && nodeRight && ['Identifier', 'MemberExpression'].includes(nodeLeft.type)) {
            ...
            if (leftCode.match(/^this\.[a-zA-Z0-9_$]+$/) || leftCode.match(/^[a-zA-Z0-9_$]+$/)) {
                const numLeftCodeRepeats = (this.funcCode.match(new RegExp(`[^\\.]${leftCode}\\.`, 'g')) || []).length;
    ...
    setPropsOrParams(codeOrNode, mockData, returns) {
        ...
        if (typeof codeOrNode === 'string') {
            nodeToUse = util_1.Util.getNode(codeOrNode);
            code = codeOrNode;
        ...
        if (map[mapKey] && params[map[mapKey]]) { // if param mapped
            if (one === 'this' && two && map[`this.${two}`]) { // if param map found
                util_1.Util.merge(obj.this, params);
            }
            else {
                // TODO : check why there was "params" in first version of the implementation
                util_1.Util.merge(code, obj);
            ...
            if (one === 'this' && two) {
                util_1.Util.merge(obj.this, props);
            }
            else if (params[one] && two) {
                util_1.Util.merge(obj, params);
            }
            else if (one === 'window' || obj === 'document') {
                util_1.Util.merge(obj, globals);
            }
        }
    }
}
exports.FuncTestGen = FuncTestGen;
