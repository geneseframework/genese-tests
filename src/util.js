...
class Util {
    ...
    static getExprMembers(code, result = []) {
        ...
        const member = /* eslint-disable */ type === 'MemberExpression' ? property.name || property.raw :
    ...
    static getFuncArgNames(code) {
        if (typeof code !== 'string')
            throw new Error('%%%%%%%%%%%%%%%%% getFuncArgNames');
        const node = Util.getNode(code);
    ...
    static getFuncParamJS(params) {
        ...
        Object.entries(params).forEach(([key2, value2]) => {
            ...
            if (key2 !== 'undefined') {
                ...
                const jsValue = objValue2 === `'ngentest'` ? `'${key2}'` :
